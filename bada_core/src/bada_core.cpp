...
void bada_roaming(int currentPoint = 0);
...
double wayPoint[][4] = {
	{-0.561, -3.943, 0.924, 0.383}, //way1
	{0.199, -0.049, 0.920, 0.391},	//way2
	{2.979, 7.445, 0.557, 0.831}	//way3
};									//roaming 장소 저장
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		case ROAMING:
			...
			waitSec(1);
			bada_roaming(0); // 카테고리에 등록된 소리가 나올 때까지 배회하기
...
void bada_roaming(int currentPoint)
{	//현재위치에서 마지막 지점까지 이동
	// bada_go_destination_blocking();			        //다음 지점으로 이동하기.
	currentPoint += 1;
	for (; currentPoint < 3; currentPoint++)
	{
		//while(ros::Duration(30))
		bada_send_destination(wayPoint[currentPoint][0], wayPoint[currentPoint][1], wayPoint[currentPoint][2], wayPoint[currentPoint][3]);
		// TODO : Investigate why while loop breaks..
		...
		while (true)
		{
			...
			{
				ROS_INFO("%d goal reach", currentPoint);
				// bada_go_destination_blocking();			//다음 지점으로 이동하기.
				break;
			}
		}
...
void bada_change_pos(float LinePos, float AnglePos)
{
	...
	do
	{
		...
		loop_rate.sleep();
	} while (ros::ok() && t * 0.1 * abs(vel) > abs(target));
}
